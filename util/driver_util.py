...
BROWSER_DRIVER_DIR = os.path.join(BASE_DIR, "drivers")
DRIVER_MAPPING_FILE = os.path.join(BASE_DIR, "conf", "mapping.json")
...
def get_latest_browser_version(browser_major_ver):
    ...
    # print("latest_api:",latest_api)
    resp = requests.get(latest_api,verify=False)
...
def download_browser_driver(latest_driver_version, browser_name):
    ...
    # print("download_api:",download_api)
    if browser_name == "Chrome":
        download_dir = os.path.join(str(BROWSER_DRIVER_DIR), browser_name.lower())
    elif browser_name == "Edge":
        download_dir = os.path.join(str(BROWSER_DRIVER_DIR), browser_name.lower())
    ...
    resp = requests.get(download_api, stream=True, timeout=30, verify=False)

    if resp.status_code == 200:
        if browser_name== "Chrome":
            with open(os.path.join(download_dir, CHROME_DRIVER_ZIP), 'wb') as fo:
                fo.write(resp.content)
        elif browser_name == "Edge":
            with open(os.path.join(download_dir, EDGE_DRIVER_ZIP), 'wb') as fo:
                fo.write(resp.content)
    else:
...
def unzip_driver(browser_major_ver, browser_name):
    ...
    driver_path = None
    if browser_name == "Chrome":
        file_path = os.path.join(BROWSER_DRIVER_DIR, browser_name.lower(), os.path.basename(CHROME_DRIVER_ZIP))
        # driver_path = os.path.join(BROWSER_DRIVER_DIR, browser_major_ver, CHROME_DRIVER)
        driver_path = os.path.join(BROWSER_DRIVER_DIR, browser_name.lower(), CHROME_DRIVER)
    elif browser_name == "Edge":
        file_path = os.path.join(BROWSER_DRIVER_DIR, browser_name.lower(), os.path.basename(EDGE_DRIVER_ZIP))
        # driver_path = os.path.join(BROWSER_DRIVER_DIR, browser_major_ver, EDGE_DRIVER)
        driver_path = os.path.join(BROWSER_DRIVER_DIR, browser_name.lower(), EDGE_DRIVER)
    browser_driver_dir = os.path.join(BROWSER_DRIVER_DIR, browser_name.lower())

    # print("file_path:",file_path)
    # print("browser_driver_dir:",browser_driver_dir)
...
def automatic_discover_driver(browser_path, browser_name="Chrome"):
    ...
    # print("browser_maj_ver:",browser_maj_ver)
    if browser_name == "Chrome":
...
if __name__ == '__main__':
    ...
    # print(DRIVER_MAPPING_FILE)
    # print(automatic_discover_driver(r"C:\Program Files\Google\Chrome\Application\chrome.exe"))
    print(automatic_discover_driver(r"C:\Program Files (x86)\Microsoft\Edge\Application\msedge.exe", browser_name="Edge"))
    # delete_driver()